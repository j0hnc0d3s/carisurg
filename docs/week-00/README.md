...
Student challenge plots focused on MAP — a histogram showing the 65 mmHg organ hypoperfusion threshold, and an Age vs MAP scatter showing the slight upward trend with age consistent with hypertension prevalence patterns. A second challenge plot (Age vs Temperature) explored whether body temperature varies with age — clinically relevant since older patients can be septic without a classic fever, so a nurse can't rely on temperature alone to rule out infection in elderly patients.
